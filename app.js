...
async function saveC(){if(!db||!S.rid)return;try{await db.collection("rooms").doc(S.rid).set({tree:S.tree,data:S.data,general:S.general,t:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});S.sync=true}catch(e){console.error(e)}}
function listenC(){if(!db||!S.rid)return;if(unsub)unsub();unsub=db.collection("rooms").doc(S.rid).onSnapshot(snap=>{if(snap.exists){const d=snap.data();S.tree=d.tree||mkTree();S.data=d.data||{};S.general=d.general||{};S.sync=true;saveL();render()}},()=>{S.sync=false})}
...
function showInviteModal(){
  ...
  S.modal=`<div class="modal" data-act="close-modal"><div class="modal-c" onclick="event.stopPropagation()"><div class="modal-t">Пригласить родственника</div><div class="modal-d">Каждый получит персональную ссылку и увидит древо от своего лица.</div>${list}<button class="btn btn-s" style="width:100%;margin-top:8px" data-act="close-modal">Закрыть</button></div></div>`;
  render()}
